...
impl<'a, E: Evented + ?Sized> mio::event::Source for EventedSource<'a, E> {
    ...
    ) -> io::Result<()>
    {
        ...
        poll.register(self.0, Token(token.0), convert_interests_to_ready(interests), PollOpt::edge())
    }
    fn reregister(
        &self, 
        registry: &mio::Registry,
    ...
    {
        let poll = Poll::from_registry(registry);
        poll.reregister(self.0, Token(token.0), convert_interests_to_ready(interests), PollOpt::edge())
    }
    fn deregister(
        &self, 
        registry: &mio::Registry
